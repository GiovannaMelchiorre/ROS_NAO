...
from nao_pkg.msg import Movement
	...
	def loop(self):
		...
		while not rospy.is_shutdown():
			...
			if self.compaire(result, previous):
				...
				self.createJson(result, "position")
				newPosition = []
				"""
				if result[0]>0:
					newPosition[0] = result[0]-previous[0]
				else:
					newPosition[0] = previous[0]-result[0]

				if result[1]>0:
					newPosition[1] = result[1]-previous[1]
				else:
					newPosition[1] = previous[1]-result[1]
				
				if result[2]>0:
					newPosition[2] = result[2]-previous[2]
				else:
					newPosition[2] = previous[2]-result[2]

				if result[3]>0:
					newPosition[3] = result[3]-previous[3]
				else:
					newPosition[3] = previous[3]-result[3]
				
				if result[4]>0:
					newPosition[4] = result[4]-previous[4]
				else:
					newPosition[4] = previous[4]-result[4]

				if result[5]>0:
					newPosition[5] = result[5]-previous[5]
				else:
					newPosition[5] = previous[5]-result[5]
